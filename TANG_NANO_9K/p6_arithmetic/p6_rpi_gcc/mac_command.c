...
https://github.com/grughuhler/grug_misc_projects/blob/main/tang_nano_mac_fpga/c/mac_command.c
...
#define CLK_PIN  21     // FPGA 35
#define CMD1_PIN 20     // FPGA 41
#define CMD0_PIN 16     // FPGA 42
#define DIN_PIN  26     // FPGA 40
#define DOUT_PIN 19     // FPGA 34

int all_input_pins[]  = {DOUT_PIN};
int all_output_pins[] = {CLK_PIN, CMD1_PIN, CMD0_PIN, DIN_PIN};
...
// These command functions require that CLK == 0 when they are called.
void do_reset(void)
{
...
void do_read(void)
{
  ...
  // dout is already the 1st bit so read before cycling the clock.
  for (i = 0; i < 2*WIDTH; i++) {
    val = (val << 1) | gpioRead(DOUT_PIN);
    cycle_clk();
  }
